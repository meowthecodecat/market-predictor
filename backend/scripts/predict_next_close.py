...
# -*- coding: utf-8 -*-
"""
Prédit uniquement le close J+1 à partir du LSTM.
Compatible avec les features d'erreur (model_errors.csv) et l'option --alpha.
"""
...
def main():
    ...
    ap.add_argument("--alpha", type=float, default=0.6)  # ignoré, compatibilité
    args = ap.parse_args()
    ...
    # features enrichies avec erreurs si dispo
    feat_df, _ = make_features(
        raw, market_df=None, earnings_dates=None, exog=exog, errors_df=err
    )
    ...
    print(
        f"{sym} -> last_close={last_close:.2f} pred_close={pred_close:.2f} d_pct={d_pct*100:.2f}"
    )
    if sigma > 0:
